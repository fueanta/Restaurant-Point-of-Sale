package gui.application.purchase.purchase_item;
...
public class PurchaseItem_Controller implements Initializable {
    ...
    public static void loadInventory(int id) {
        PurchaseItem_Controller.id = id;
        window = new Stage();
        window.setTitle("Purchase Items");
        window.setResizable(false);
        window.initModality(Modality.APPLICATION_MODAL);
        window.getIcons().add(new Image("/resource/image/spoons.png"));
        window.setScene(getPurchaseItemScene());
        window.showAndWait();
    }

    public static Scene getPurchaseItemScene() {
        Scene purchaseItem = null;
        try {
            Parent root = FXMLLoader.load(PurchaseItem_Controller.class.getResource("PurchaseItem_GUI.fxml"));
            purchaseItem = new Scene(root);
        } catch (IOException e) {
            System.out.println("Purchase Item GUI could not be loaded!");
        }
        return purchaseItem;
    }
    ...
    private void putDataOnList() {
        list1 = DB_DataFetcher.getOtherPurchaseSubs(id);
        list2 = DB_DataFetcher.getPurchaseSubs(id);
    }

    public void updateBtnOnAction(ActionEvent event) {
        ObservableList<ModifiedMainSub> list = table2.getItems();
        DB_DataModifier.update_purchase_item(list, id);
        window.close();
    }
