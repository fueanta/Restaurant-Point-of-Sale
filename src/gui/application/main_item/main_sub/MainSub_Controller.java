package gui.application.main_item.main_sub;
...
public class MainSub_Controller implements Initializable {
    ...
    public static void loadInventory(int id) {
        MainSub_Controller.id = id;
        window = new Stage();
        window.setTitle("Inventory Items");
        window.setResizable(false);
        window.initModality(Modality.APPLICATION_MODAL);
        window.getIcons().add(new Image("/resource/image/spoons.png"));
        window.setScene(getMainSubScene());
        window.showAndWait();
    }

    public static Scene getMainSubScene() {
        Scene mainSubScene = null;
        try {
            Parent root = FXMLLoader.load(MainSub_Controller.class.getResource("MainSub_GUI.fxml"));
            mainSubScene = new Scene(root);
        } catch (IOException e) {
            System.out.println("Main Sub GUI could not be loaded!");
            e.printStackTrace();
        }
        return mainSubScene;
    }
    ...
    private void putDataOnList() {
        list1 = DB_DataFetcher.getOtherSubs(id);
        list2 = DB_DataFetcher.getSubs(id);
    }
    ...
    public void updateBtnOnAction(ActionEvent event) {
        ...
        DB_DataModifier.update_main_sub(list, id);
        window.close();
    }
